...
本报告记录 Task 19 文档、验收和打包体检结果。验收日期为 2026-06-25，工作区为 `E:\Automatic\personal_wiki_agent`。
...
- Task 18 Web UI 已落地到 `frontend/`：提供 React + Vite + TypeScript 对话式 Agent 工作台，并通过前端测试和 TypeScript 类型检查；生产构建输出写入需要在普通本地环境复验。
...
| 验证命令 | 当前结果 | 说明 |
| --- | --- | --- |
| `.\.venv\Scripts\python.exe -m pytest backend/tests -q` | 通过；`76 passed` | 用于验证 Task 1 到 Task 17 后端能力。 |
...
| `.\.venv\Scripts\python.exe -m pytest backend/tests/test_chat_api.py -q` | 通过；`4 passed` | 用于回归验证 Chat API 引用、无来源保护和模型配置错误。 |
| `npm.cmd test` | 通过；3 个测试文件、5 个测试通过 | 用于验证 Task 18 前端 API client、工具活动流和对话视图。 |
...
| 本地 Markdown 链接解析脚本 | 通过；`OK: local markdown links resolve` | 用于检查 README、docs 和 rules 中的本地相对链接目标存在。 |
...
| Task 2 配置系统 | 可读取 YAML 配置，支持本地目录、同步笔记目录、Obsidian vault、模型和忽略规则。 | `backend/tests/test_settings.py`；检查 `config/sources.example.yaml`。 | 通过。 | 当前配置文件读取能力未直接形成 Source 管理 HTTP API，后续需补 `GET /sources`、`POST /sources`。 |
...
| Task 9 索引流水线 | 可扫描 source、解析文件、写入 document/chunk/job，并可接入 FTS。 | `backend/tests/test_indexing_pipeline.py`。 | 通过。 | 当前索引入口是内部 `IndexingPipeline`，HTTP `POST /index/run` 待补齐。 |
...
| Task 14 Search API 与来源详情 | `POST /search`、`GET /documents/{document_id}`、`GET /chunks/{chunk_id}` 返回可追溯结果。 | `backend/tests/test_search_api.py`。 | 通过。 | `GET /sources`、`POST /sources` 尚未注册。 |
| Task 15 Chat API | `POST /chat` 返回 `answer`、`citations`、`memories_used`、`confidence`、`retrieval_summary`；无可靠来源时不伪造引用。 | `backend/tests/test_chat_api.py`、`backend/tests/test_memory.py`。 | 通过。 | 真实模型 provider HTTP 调用仍需后续验证。 |
...
| Task 17 Memory API | `POST /memory` 创建记忆；`GET /memory` 按 query、memory_type、limit 查询 active 且未过期记忆；Chat 响应区分 `citations` 和 `memories_used`。 | `backend/tests/test_memory.py`；后端全量测试。 | 通过；`test_memory.py` 6 passed，全量后端测试 76 passed。 | 后续需在 Web UI 中提供记忆管理入口，并继续保持文档引用与记忆上下文分离。 |
| Task 18 Web UI | `frontend/` React + Vite + TypeScript 对话式 Agent 工作台，包含对话页、引用抽屉、工具活动流、数据源只读入口和索引任务只读入口。 | `npm.cmd test`；`npm.cmd exec tsc -- --noEmit`；`npm.cmd run build`。 | 部分通过；3 个测试文件、5 个测试通过，TypeScript 类型检查通过；生产构建输出写入被当前沙箱 Node 权限拦截。 | 需要在普通本地环境复验 `npm.cmd run build`，并继续补充浏览器端到端手动验收。 |
...
- 文档知识库与长期记忆在模型层保持分离。
- Web UI 已形成对话式工作台，并通过前端测试和 TypeScript 类型检查。
- 项目文档已补充当前状态、待集成边界、运行命令和验收报告。

## 未完成能力

- Source 管理 API：`GET /sources`、`POST /sources`。
- 索引任务 API：`POST /index/run`、`GET /index/jobs`。
- Web UI 生产构建输出写入、浏览器端到端手动验收和真实后端联调。
...
| [technical-direction.md](technical-direction.md) | 已补充 Task 18 / Task 19 当前实现状态，并把下一步修正为 Source / Index API 与真实端到端联调。 | 后续路线变化时继续作为 source of truth。 |
| [project-design.md](project-design.md) | 已区分 MVP 目标 API 和当前已注册 API，补充 Memory 最终契约和 Web UI 已验证状态，并链接本报告。 | Source、Index API 合并后更新当前实现状态。 |
| [mvp-implementation-plan.md](mvp-implementation-plan.md) | 已修正过期的“优先开工 Task 1 到 Task 4”，补充 Task 17 和 Task 18 完成记录，并改为当前 Source / Index API 推进顺序。 | 后续端到端验收后更新对应任务状态。 |
...
本次已验证 `npm.cmd test` 和 `npm.cmd exec tsc -- --noEmit`。`npm.cmd run build` 在当前沙箱中被 Node 写文件权限拦截，需要在普通本地环境复验。`npm run dev` 用于本地手动浏览器验收，本报告不保留常驻开发服务器。

## 后续动作

1. 将 `IndexingPipeline` 封装为 HTTP 索引任务 API。
2. 补齐 Source 管理 API，让配置、数据库和 UI 数据源入口对齐。
3. 将 Web UI 的数据源页和索引任务页接入真实 API。
4. 运行浏览器端到端手动验收，补充真实本地目录索引、搜索、问答、来源抽屉和记忆使用结果。
